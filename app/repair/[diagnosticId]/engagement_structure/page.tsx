...
export default async function EngagementPassPage({
  params,
}: {
  ...
  if (!user)
    redirect(`/login?next=/repair/${diagnosticId}/engagement_structure`);
      ...
      <main className="flex-1 px-6 py-8 max-w-4xl mx-auto w-full space-y-6">
        ...
        <header className="space-y-2">
          ...
          <h1 className="text-2xl font-semibold tracking-tight">
            {PASS_LABEL.engagement_structure}
          </h1>
          <p className="text-sm text-muted-foreground leading-relaxed">
            {PASS_BLURB.engagement_structure}
          </p>
          <p className="text-xs text-muted-foreground">
            Addresses:{" "}
            {PASS_DIMENSIONS.engagement_structure
              .map((d) =>
                d
                  .replace(/_/g, " ")
