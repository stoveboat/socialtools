...
import { FoundationFlow } from "./foundation-flow";

export default async function FoundationPassPage({
  params,
}: {
  ...
  if (!user) redirect(`/login?next=/repair/${diagnosticId}/foundation`);
  ...
  const { data: ctx } = await supabase
    ...
    .select(
      "audience_selection, custom_audience",
    )
      ...
      <main className="flex-1 px-6 py-8 max-w-4xl mx-auto w-full space-y-6">
        ...
        <header className="space-y-2">
          ...
          <h1 className="text-2xl font-semibold tracking-tight">
            {PASS_LABEL.foundation}
          </h1>
          <p className="text-sm text-muted-foreground leading-relaxed">
            {PASS_BLURB.foundation}
          </p>
          <p className="text-xs text-muted-foreground">
            Addresses:{" "}
            {PASS_DIMENSIONS.foundation
              .map((d) =>
                d
                  .replace(/_/g, " ")
